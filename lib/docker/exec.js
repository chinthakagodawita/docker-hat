...
exports.run = function (name, cmd, opts) {
  ...
  if (opts === '-d') {
    // Run detached changes in a non-tty shell. child_process.spawn() doesn't
    // execute our commands if detached sometimes.
    ret = libUtils.common.shell('docker', args, {
      silent: true
    })
    retCode = ret.code;
  }
  else {
    // We need a tty for non-detached execs which shelljs doesn't let us do.
    // Run these through child_process.spawn() instead.
  ...
  // Ignore success for interactive/TTY runs as they're not scripted.
  if (opts == '-it' || retCode === 0) {
    return true;
  }
...
exports.pull = function (image) {
  libUtils.logger.info("Pulling Docker image for '" + image + "'.")
