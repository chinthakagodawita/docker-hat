...
exports.setupNfs = function () {
  ...
  // Write to temporary file and then pipe that to the exports file. Attempting
  // to write or echo this through shellJS causes errors due to the contents
  // of the string. We can't write via `fs` either due to permission
  // restrictions (need sudo).
  exportsTmp = libUtils.common.writeTempFile(exportsTpl);
  libUtils.common.shell('cat ' + exportsTmp + ' | sudo tee -a ' + exportsFile, [], {
    silent: true
  });
