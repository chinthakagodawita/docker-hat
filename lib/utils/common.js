...
exports.checkRequirements = function () {
  ...
  // @OTODO: Only check the following if on a Mac:
...
exports.runSubscript = function (scriptname, yargs) {
  ...
  args = args.concat(this.getYargsFlags(yargs.argv));
...
exports.getYargsFlags = function (argv, ignoredFlags) {
  var flags = [];

  if (typeof ignoredFlags === 'undefined') {
    ignoredFlags = ['h', 'help'];
  }

  // Get all valid flags defined in the yargs instance.
  for (var flag in argv) {
    ...
    if (flag.indexOf('$') > -1 || flag == '_') {
      continue;
    }

    // Skip all ignored flags.
    if (ignoredFlags.indexOf(flag) > -1) {
      continue;
    }
    ...
    flagDef = flagType + flag;

    // Don't send boolean flags as strings.
    flagValue = argv[flag];
